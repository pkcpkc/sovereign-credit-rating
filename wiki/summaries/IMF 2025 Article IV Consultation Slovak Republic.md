---
type: Summary
title: "IMF 2025 Article IV Consultation: Slovak Republic"
tags:
  - Slovakia
  - IMF Article IV
  - Fiscal Consolidation
  - Euro Area
  - Debt Sustainability
  - Structural Reforms
assets:
  - wiki/assets/20260705-000000/processed/IMF (2025) Slovakia Country Report.md
  - wiki/assets/20260705-000000/sources/IMF (2025) Slovakia Country Report.md
concepts:
  - fiscal policy
  - debt sustainability
  - macroeconomic stability
  - banking sector health
  - external vulnerability
  - monetary policy
  - institutional strength
  - structural reforms
institutions:
  - IMF
  - National Bank of Slovakia
  - European Commission
  - Council for Budgetary Responsibility
  - Ministry of Finance
methods:
  - Article IV Consultation
  - External Sector Assessment (EBA-lite)
  - Financial Stability Assessment Program (FSAP)
  - Debt Sustainability Analysis (DSA)
persons:
  - name: Oya Celasun
    role: Department Head
    organization: IMF (EUR)
    countries_covered: []
  - name: Stefania Fabrizio
    role: Department Head
    organization: IMF (SPR)
    countries_covered: []
  - name: Saxegaard
    role: Mission Head
    organization: IMF
    countries_covered:
      - Slovakia
  - name: Kažimír
    role: Governor
    organization: National Bank of Slovakia
    countries_covered:
      - Slovakia
  - name: Kamenický
    role: Minister of Finance
    organization: Government of Slovakia
    countries_covered:
      - Slovakia
  - name: Taraba
    role: Minister of the Environment
    organization: Government of Slovakia
    countries_covered:
      - Slovakia
sovereign-credit-rating-factors:
  - name: Slovakia
    geopolitical_and_political_stability:
      threats:
        - Intensifying trade policy uncertainty
        - Geoeconomic fragmentation
        - Proximity to regional conflicts (e.g., Ukraine)
        - Political polarization complicating policymaking
        - Governance concerns potentially slowing EU fund disbursements
      strengths:
        - EU and Eurozone membership providing institutional anchoring
        - Peaceful regional relations generally, despite proximity to conflict
    policy_credibility_and_institutional_strength:
      threats:
        - Perception of widespread corruption lagging behind EU averages
        - Judicial independence concerns
        - Risk of disruptive fiscal consolidation if debt brake triggers
          unexpectedly
        - Mixed implementation of past policy advice
      strengths:
        - Strong and independent Council for Budgetary Responsibility
        - Adoption of ambitious 4-year fiscal consolidation plan
        - High data adequacy and transparency (SDDS Plus)
    policymaking_stability:
      threats:
        - Political polarization
        - Lack of political consensus on structural reforms
        - Recent government change impacting budget preparation time
      strengths:
        - Established fiscal rules and expenditure ceiling framework
    accountability:
      threats:
        - Decline in perceived effectiveness of anti-corruption policies
        - Judicial safeguards for independence need strengthening
        - Transparency issues in public procurement
      strengths:
        - Existing asset declaration systems for high-risk officials
    debt_payment_culture:
      threats:
        - None explicitly mentioned
      strengths:
        - Euro area membership eliminates default risk in local currency terms
        - Strong historical commitment to honoring obligations
    economic_strength_and_tax_base:
      threats:
        - Relatively low GDP per capita compared to advanced EU peers
        - Vulnerability of automotive sector to green transition and automation
      strengths:
        - Integration into global value chains
        - Broad revenue base with high tax-to-GDP ratio (over 40%)
    growth_prospects:
      threats:
        - Adverse demographic trends (aging population)
        - Slowing productivity growth
        - Weak external demand (especially from Germany)
      strengths:
        - Resilient near-term growth outpacing Euro Area average
        - Potential for productivity gains through innovation and EU funds
    economic_diversity:
      threats:
        - Heavy dependence on automotive sector
        - Low economic complexity compared to peers
      strengths:
        - Some horizontal diversification across automotive groups
    external_liquidity:
      threats:
        - Negative Net International Investment Position (NIIP)
        - High gross external debt (approx. 100% of GDP)
      strengths:
        - Eurozone membership ensures ample liquidity access
        - High share of long-term, euro-denominated debt reduces refinancing risk
        - Strong FDI inflows
    currency_status:
      threats:
        - None (Eurozone member)
      strengths:
        - Euro is a global reserve currency
    external_asset_liability_position:
      threats:
        - Negative NIIP (-53.5% of GDP)
        - High gross liabilities (144% of GDP)
      strengths:
        - NIIP improving due to strong GDP growth and high inflation
        - High share of FDI liabilities which are stable
    financial_sector_health:
      threats:
        - Vulnerabilities in commercial real estate (CRE), specifically office
          segment
        - Refixation risk in residential mortgages in coming years
        - High household debt levels
      strengths:
        - High capital adequacy ratios well above regulatory minima
        - Ample liquidity buffers (LCR > 200%)
        - Low non-performing loans (NPLs)
        - Resilient banking sector per FSAP stress tests
    fiscal_performance_and_flexibility:
      threats:
        - Large fiscal deficit (5.8% of GDP in 2024)
        - Excessive Deficit Procedure (EDP) initiated by EU
        - Reliance on revenue-based consolidation measures (e.g., FTT)
      strengths:
        - Ambitious consolidation targets for 2025-2028
        - Scope for expenditure rationalization and VAT base broadening
    debt_burden:
      threats:
        - Rising public debt (58% of GDP in 2024, projected to rise)
        - Heavy interest burden relative to primary balance
      strengths:
        - Debt is primarily in local currency (Euro)
        - Long average residual maturity (approx. 8 years)
    fiscal_sustainability:
      threats:
        - High long-term risk of sovereign stress due to aging costs
        - Projected aging-related costs to increase by 5.6% of GDP by 2050
        - Persistent primary deficits
      strengths:
        - Medium-term consolidation plan aims to reverse debt trajectory
    contingent_liabilities:
      threats:
        - Potential banking sector support costs if CRE/NPLs rise
        - State-owned enterprise liabilities
      strengths:
        - Strong banking sector reduces immediate recapitalization risk
    price_stability:
      threats:
        - Temporary inflation surge in 2025 (4.1%) due to tax increases
        - Higher core inflation than Euro Area median due to wage growth
      strengths:
        - Inflation expected to return to 2% target by early 2027
        - Medium-term inflation expectations anchored
    central_bank_independence:
      threats:
        - Potential political pressure on fiscal/monetary outcomes
      strengths:
        - National Bank of Slovakia operates within ECB framework
        - Supervisory powers being strengthened
    exchange_rate_regime:
      threats:
        - None (Eurozone member)
      strengths:
        - Full integration into Eurozone eliminates exchange rate risk
times:
  - date: 2025-03-24
    title: IMF Executive Board concludes 2025 Article IV consultation with Slovak
      Republic
  - date: 2025-03-18
    title: IMF Executive Board consideration of staff report
  - date: 2025-03-03
    title: Staff report completed
  - date: 2025-01-28
    title: Discussions with Slovak officials ended
  - date: 2024-12-04
    title: Virtual mission discussions began
  - date: 2024-10
    title: Change of government in Slovakia
  - date: 2024-07
    title: European Council initiates Excessive Deficit Procedure against Slovakia
  - date: "2024"
    title: Two FSAP missions to Slovak Republic
generated:
  by: mycelium-mind-migration-v0.2
  at: 2026-07-05T11:29:03Z
status: stable
sources:
  - resource: /assets/wiki/assets/20260705-000000/processed/IMF (2025) Slovakia
      Country Report.md
    title: wiki/assets/20260705-000000/processed/IMF (2025) Slovakia Country Report.md
  - resource: /assets/wiki/assets/20260705-000000/sources/IMF (2025) Slovakia
      Country Report.md
    title: wiki/assets/20260705-000000/sources/IMF (2025) Slovakia Country Report.md
---
# Summary of: IMF 2025 Article IV Consultation: Slovak Republic

## Context Metadata

- **Source Document**: IMF Staff Report for the 2025 Article IV Consultation with the Slovak Republic
- **Effective Period**: March 2025 (covering economic developments through early 2025)
- **Issuer**: International Monetary Fund (IMF)
- **Document Type**: Article IV Consultation Staff Report and Press Release

## Executive Summary

The IMF’s 2025 Article IV Consultation with the Slovak Republic assesses an economy recovering from recent headwinds, with growth accelerating to 2.0% in 2024, outpacing the Euro Area average. However, this recovery has been accompanied by a significant widening of the fiscal deficit, which reached 5.8% of GDP in 2024, triggering an Excessive Deficit Procedure (EDP) by the European Commission. The IMF staff and Executive Board endorse the Slovak government’s ambitious four-year fiscal consolidation plan (2025–2028), which aims to reduce the deficit to approximately 2.2% of GDP by 2028. The staff projects that while the 2025 budget measures are a step in the right direction, further deficit-reducing measures—particularly on the expenditure side—are required to meet medium-term targets and put public debt on a sustainable downward path.

Key risks to the outlook are tilted to the downside, driven by trade policy uncertainty, potential slowdowns in EU fund disbursements due to governance concerns, and structural challenges including an aging population and dependence on the automotive sector. The financial sector remains resilient with strong capital buffers, though vulnerabilities persist in the commercial real estate sector and from upcoming mortgage refixations. The report emphasizes the need for structural reforms to diversify the economy, enhance productivity, improve governance, and address long-term fiscal sustainability pressures from demographic aging.

## Key Highlights

- **[Fiscal Consolidation Targets]**: The Slovak government has proposed a 4-year consolidation plan to reduce the fiscal deficit from 5.8% of GDP in 2024 to 2.2% by 2028. The IMF staff supports this ambitious agenda as commensurate with Slovakia's fiscal challenges but notes that additional measures beyond the 2025 budget will be needed, prioritizing expenditure-based savings (e.g., reducing subsidies, targeting social spending) over further revenue hikes.
- **[Growth and Inflation Outlook]**: Real GDP growth is projected to moderate to 1.8% in 2025 due to fiscal consolidation and weak external demand, before recovering to 2.1% in 2026. Inflation is expected to rise temporarily to 4.1% in 2025 driven by indirect tax increases, then decline to the 2% target by early 2027.
- **[Debt Sustainability and Risks]**: Public debt is projected to rise in the medium term, reaching 74.3% of GDP by 2030 under the baseline, with a moderate overall risk of sovereign stress. However, long-term risk is assessed as high due to significant aging-related costs, which could increase fiscal burdens by 5.6% of GDP by 2050 without further reforms.
- **[Financial Sector Resilience]**: The banking sector is deemed resilient, with capital adequacy ratios well above regulatory minima and ample liquidity. Stress tests confirm resilience to severe shocks. Primary vulnerabilities lie in the commercial real estate (CRE) market, particularly the office segment, and the refixation of residential mortgages in the coming years. The IMF recommends strengthening the macroprudential framework, including adopting a positive neutral countercyclical capital buffer.
- **[Structural Reforms and Governance]**: Slovakia faces structural headwinds from geoeconomic fragmentation and demographics. The report calls for reforms to diversify the economy away from automotive dependence, boost innovation, and increase labor force participation. Crucially, strengthening governance, reducing corruption vulnerabilities, and enhancing judicial independence are highlighted as essential to maintaining investor confidence and ensuring the effective absorption of EU funds.
- **[External Sector Position]**: The current account deficit widened to 2.7% of GDP in 2024, assessed as moderately weaker than fundamentals. The position is expected to improve as fiscal consolidation proceeds and EU funds support investment. The Net International Investment Position (NIIP) remains negative (-53.5% of GDP) but is improving, with high FDI inflows mitigating risks of sudden capital outflows.